...
const generate = async (first, last, after, before, model) => {
...
const generateWithQuery = async (first, last, after, before, model, query) => {
	...
	if (first) { // HAY FIRST
		...
		if (before && after) {
			...
			allDocuments = await model.find(cursorOptions).exec();
